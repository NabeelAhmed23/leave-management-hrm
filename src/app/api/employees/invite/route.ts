...
/**
 * POST /api/employees/invite - Send employee invite
 */
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    logger.info(`Sending employee invite to: ${validatedData.email}`);

    // Get employee ID from request body or URL
    const { searchParams } = new URL(request.url);
    const employeeId = body.employeeId || searchParams.get("employeeId");
    ...
    // Send invite
    const result = await sendEmployeeInvite(employeeId, validatedData);

    logger.info(`Employee invite sent successfully to: ${validatedData.email}`);

    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    logger.error("Error in POST /api/employees/invite:", {
